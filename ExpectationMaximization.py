...
def init(points_list, k):
    ...
    """
    w = np.array([1 / k] * k)
    mu = np.mean(np.array_split(points_list, k, axis=0), axis=1)
    sigma = np.std(np.array_split(points_list, k, axis=0), axis=1)
...
def expectation(points_list, mu, sigma, w):
    ...
    """
    likelihood = np.zeros(shape=(len(points_list), len(w)))
    for i in range(len(points_list)):
        for j in range(len(w)):
            current_likelihood = normal_pdf(points_list[i], mu[j], sigma[j], w[j])
            likelihood[i][j] = (w[j] * current_likelihood)
...
def normal_pdf(x, mu, sigma, w):
    ...
    """
    EPSILLON = 1e-10
    e_exp = - ((np.square(x - mu)) / (2 * np.square(sigma)))

    if (np.sqrt(2 * np.pi * np.square(sigma))) == 0:
        return EPSILLON
    else:
        return ((np.power(np.e, e_exp)) / (np.sqrt(2 * np.pi * np.square(sigma))))
...
def maximization(points_list, ranks):
    ...
    w_new = (1 / ranks.shape[0]) * np.sum(ranks, axis=0)

    sum_in_mu = ranks * (np.array(points_list))[:, np.newaxis]
    mu_new = (1 / (ranks.shape[0] * w_new)) * np.sum(sum_in_mu, axis=0)

    sigma_new = np.array([0.0] * len(ranks[0]))
    for j in range(len(w_new)):
        for i in range(len(points_list)):
            sigma_new[j] += ranks[i][j] * np.square(points_list[i] - mu_new[j])
        sigma_new[j] = np.sqrt(sigma_new[j] / (len(ranks) * w_new[j]))
...
def calc_max_delta(old_param, new_param):
    ...
    """
    return np.max(np.abs(new_param - old_param))
...
def expectation_maximization(points_list, k, max_iter, epsilon):
    ...
    while delta > epsilon and iter_num <= max_iter:
        ...
        # M step
        ranks = likelihood / (np.reshape(likelihood_sum, (likelihood_sum.size, 1)))
